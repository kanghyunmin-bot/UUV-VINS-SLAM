...
int USE_IMU;
int MULTIPLE_THREAD;
double STEREO_SYNC_TOLERANCE;
int DROP_OLD_FRAMES;
int IMAGE_QOS_DEPTH;
int IMAGE_QOS_RELIABLE;
...
void readParameters(std::string config_file)
{
    ...
    if (!fsSettings["stereo_sync_tolerance"].empty())
        STEREO_SYNC_TOLERANCE = static_cast<double>(fsSettings["stereo_sync_tolerance"]);
    DROP_OLD_FRAMES = 0;
    if (!fsSettings["drop_old_frames"].empty())
        DROP_OLD_FRAMES = static_cast<int>(fsSettings["drop_old_frames"]);
    IMAGE_QOS_DEPTH = 2000;
    if (!fsSettings["image_qos_depth"].empty())
        IMAGE_QOS_DEPTH = static_cast<int>(fsSettings["image_qos_depth"]);
    IMAGE_QOS_RELIABLE = 1;
    if (!fsSettings["image_qos_reliable"].empty())
        IMAGE_QOS_RELIABLE = static_cast<int>(fsSettings["image_qos_reliable"]);
